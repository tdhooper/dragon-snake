...
// var mesh = new THREE.Mesh(ico, wireframeMaterial);
// scene.add(mesh);

var drawnLines = [];

var lineHash = function(a, b) {
...
var edgeGeom = new THREE.CylinderGeometry(
  lineWidth,
  lineWidth,
  lineHeight,
  lineSegments,
...
var icoLines = new THREE.Object3D();
scene.add(icoLines);

var lineMesh = new THREE.Mesh(edgeGeom, materialA);
var sphereMeshA = new THREE.Mesh(sphereGeom, materialA);
var sphereMeshB = sphereMeshA.clone();
sphereMeshA.position.y = lineHeight * 0.5;
sphereMeshB.position.y = lineHeight * -0.5;

lineObj = new THREE.Object3D();
lineObj.add(lineMesh);
lineObj.add(sphereMeshA);
lineObj.add(sphereMeshB);

var addLine = function(a, b) {
  var hash = lineHash(a, b);
  if (drawnLines.indexOf(hash) > -1) {
    return;
  }
  var mesh = lineObj.clone();
  mesh.position.copy(new THREE.Vector3().lerpVectors(
    ico.vertices[a],
    ico.vertices[b],
    0.5
  ));
  mesh.up.copy(ico.vertices[b].clone());
  mesh.lookAt(new THREE.Vector3(0, 0, 0));
  icoLines.add(mesh);
  drawnLines.push(hash);
};

for (var i = 0; i < ico.faces.length; i++) {
  var face = ico.faces[i];
  addLine(face.a, face.b);
  addLine(face.a, face.c);
  addLine(face.b, face.c);
}
